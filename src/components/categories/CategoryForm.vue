      ...
      <q-card-section class="q-pt-sm q-gutter-md">
        ...
        <div>
          <div class="form-label">
            {{ $t('category_form.name') }} <span class="text-negative">*</span>
          </div>
          <q-input
            v-model="form.name"
            outlined
        ...
        <div>
          <div class="form-label">
            {{ $t('category_form.icon') }} <span class="text-negative">*</span>
          </div>
          <q-select
            v-model="form.icon"
            :options="iconOptions"
        ...
        <div>
          <div class="form-label">
            {{ $t('category_form.color') }} <span class="text-negative">*</span>
          </div>
        ...
        <div>
          <div class="form-label">
            ...
            <q-icon name="help_outline" size="14px" color="grey-5" class="q-ml-xs cursor-pointer">
        ...
        <q-card flat class="preview-card">
          <q-card-section class="row items-center q-py-sm q-px-md">
            <div class="preview-icon" :style="{ backgroundColor: form.color }">
              <q-icon :name="form.icon" color="white" size="20px" />
            </div>
            <div class="q-ml-sm">
              <div class="text-body2 text-weight-bold">
                {{ $t('category_form.preview') }}
              </div>
              <div class="text-caption text-grey-6">
                {{ form.name || $t('category_form.name_placeholder_empty') }}
              </div>
            </div>
          </q-card-section>
        </q-card>
...
function resetForm() {
  form.value = props.category
    ? {
        ...
        budget: props.category.budget ?? null,
      }
    : defaultForm()
}
...
function handleSubmit() {
  ...
  if (props.category) {
    ...
    $q.notify({
      message: t('notify.category_updated'),
      color: 'positive',
      icon: 'check_circle',
      position: 'bottom',
      timeout: 2500,
    ...
    $q.notify({
      message: t('notify.category_added'),
      color: 'positive',
      icon: 'check_circle',
      position: 'bottom',
      timeout: 2500,
